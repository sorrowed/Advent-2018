...
class Time:
    ...
    def value(self):
        return (self.year << 48) | (self.month << 32) | (self.day << 16) | (self.hour << 8) | self.minute

    def __str__(self):
        return "{0:04d}-{1:02d}-{2:02d}T{3:02d}:{4:02d}".format(self.year, self.month, self.day, self.hour, self.minute)
...
class GuardEvent:
    ...
    def __init__(self):
        self.guard_id = -1
        self.tm = Time()
        self.ev = Event.INVALID

    def __str__(self):
        return "{0:04d} {1} {2}".format(self.guard_id, self.tm, self.ev)


class Guard:
    def __init__(self):
        self.id = -1
        self.event = GuardEvent()
        self.asleep = dict()
        self.total_asleep = 0

    def __str__(self):
        return "{0:04d} {1} {2}".format(self.id, self.event, self.asleep)
...
def parse_guard_event(line):
    ...
    tm = Time()
    tm.year = int(matches.group(1))
    tm.month = int(matches.group(2))
    tm.day = int(matches.group(3))
    tm.hour = int(matches.group(4))
    tm.minute = int(matches.group(5))

    guard_id = -1
    event = Event.BEGIN_SHIFT

    if [True for g in matches.groups() if g is None or len(g) == 0]:
        event = {"wakes up": Event.WAKE_UP, "falls asleep": Event.FALL_ASLEEP}[matches.group(6)]
    else:
        guard_id = int(matches.group(7))
...
def prepare_guards():
    events = list()
    for line in input:
        guard = parse_guard_event(line)
        events.append(guard)

    events.sort(key=lambda ev: ev.tm.value())
    ...
    for event in events:
        if event.guard_id != -1:
            ...
            guard.id = guard_id
            guard.event = None
            guard.asleep = {i: 0 for i in range(0, 59)}
    ...
    asleep_time = None
    for ev in events:
        if ev.ev == Event.FALL_ASLEEP:
            asleep_time = ev.tm
        elif ev.ev == Event.WAKE_UP:
            for m in range(asleep_time.minute, ev.tm.minute):
                guards[ev.guard_id].asleep[m] += 1

    for guard in guards.values():
        guard.total_asleep = sum(guard.asleep.values())
        guard.most_asleep = max(guard.asleep, key=lambda x: guard.asleep[x])

    return guards


def first():
    guards = prepare_guards()

    guard = guards[max(guards, key=lambda x: guards[x].total_asleep)]

    return guard.id * guard.most_asleep


def second():
    guards = prepare_guards()

    # Selects the guard based on who was most frequently asleep in a specific minute
    
    guard = guards[max(guards, key=lambda x: guards[x].asleep[guards[x].most_asleep])]

    return guard.id * guard.most_asleep


if __name__ == "__main__":
    print("Guard id * minute most asleep {0}".format(first()))
    print("Guard id * times most asleep {0}".format(second()))
